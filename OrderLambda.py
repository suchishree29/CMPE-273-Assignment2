...
def lambda_handler(event,context):
    method = event['method']
    item = getOrderMenuItems(event,context)
    
    if((item != None) and method == "POST"):
        result = postPizzaOrder(event,context,item)
    elif(method == "PUT"):
        result = putPizzaSelection(event,context)
    elif(method == "GET"):
        result = getPizzaOrder(event,context)
    else:
        result = "Menu doesn't exist"
    return result
    
def getOrderMenuItems(event,context):
...
def postPizzaOrder(event,context,item):
    menu_id = event['body']['menu_id']
...
def putPizzaSelection(event,context):
...
def getPizzaOrder(event, context):
    order_id = event['params']['order-id']
    try:
        result = order_table.get_item(
            Key= {
        ...
        response = "Order Id do not exist"
    return response